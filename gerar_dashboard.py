...
kpis = {**load_json("kpis_eda.json"), **load_json("kpis_predictive.json"), **load_json("kpis_cleaning.json")}
...
kpi_cards = [
    ...
    ("🧹", f"{kpis['price_outliers_detected']}", "Price outliers capped (IQR)"),
...
sections = [
    ...
    ("08_brand_summary_table", "Grouped Brand Summary", "Aggregated metrics per brand — model count, average price, RAM, battery, camera, 5G and dual-SIM share — produced with a pandas groupby."),
...
<body>
  <header>
    ...
    <p>Validated, cleaned and grouped with pandas/NumPy, then modeled with
